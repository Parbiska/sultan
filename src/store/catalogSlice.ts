...
export const requestProducts = createAsyncThunk<string, {}, ThunkApiConfig>(
	...
	async (arg, { dispatch, getState }) => {
		...
		const { minPrice, careType, maxPrice, searchWords, brands, companies } =
			state.catalog.searchSelectors;
		...
		setTimeout(() => {
			...
			if (companies.length) {
				resultArray = resultArray.filter((el) =>
					companies.includes(el.company)
				);
			}

			if (brands.length) {
				resultArray = resultArray.filter((el) =>
					brands.includes(el.brand)
				);
			}

			if (searchWords !== undefined) {
				resultArray = resultArray.filter((el) =>
					`${el.brand} ${el.company} ${el.description} ${el.name}`
						.toLocaleLowerCase()
...
const catalogSlice = createSlice({
	...
	reducers: {
		...
		setSearchSelectors(
			state,
			action: {
				payload: SearchSelectors;
				type: string;
			}
		) {
			console.log(action.payload);
			state.searchSelectors = action.payload;
		},
	},
...
export const { loadProducts, clearProductsList, setSearchSelectors } =
